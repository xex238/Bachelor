...
GLUnurbsObj *theNurb;
GLfloat ctlarray[3][2][4] = { 0.5, 0.0, -0.5, 1.0, 0.5, 0.0, 0.5, 1.0, 0.0, 0.866 * 0.55,
							  0.5 * 0.55, 1.0 * 0.55,
							  0.0, 0.866 * 0.55, 0.5 * 0.55, 1.0 * 0.55, -0.5, 0.0, -0.5, 1.0, -0.5, 0.0, 0.5, 1.0 };

//GLfloat ctlarray2[3][2][4] =
//{ 0.0, -sqrt(3) * 0.5, -0.5, 1.0,
//0.0, -sqrt(3) * 0.5, 0.5, 1.0,
//-1.0 * 0.5, -sqrt(3) * 0.5 * 0.5, -0.5 * 0.5, 1.0 * 0.5,
//-1.0 * 0.5, -sqrt(3) * 0.5 * 0.5,  0.5 * 0.5, 1.0 * 0.5,
//-0.5, 0.0, -0.5, 1.0,
//-0.5, 0.0, 0.5, 1.0 };
//
//GLfloat ctlarray3[3][2][4] =
//{ 0.0, -sqrt(3) * 0.5, -0.5, 1.0,
//0.0, -sqrt(3) * 0.5, 0.5, 1.0,
//1.0 * 0.5, -sqrt(3) * 0.5 * 0.5, -0.5 * 0.5, 1.0 * 0.5,
//1.0 * 0.5, -sqrt(3) * 0.5 * 0.5,  0.5 * 0.5, 1.0 * 0.5,
...
void file_read()
{
	...
	//errno_t err = fopen_s(&stream, "Cute.bmp", "rb");
	errno_t err = fopen_s(&stream, "image.bmp", "rb");
	//errno_t err = fopen_s(&stream, "lotos.bmp", "rb");
...
void Display()
{
	...
	//gluBeginSurface(theNurb);
	//gluNurbsSurface(theNurb, 6, knot, 4, knot1, 2 * 4, 4, &ctlarray2[0][0][0], 3, 2, GL_MAP2_VERTEX_4);
	//gluEndSurface(theNurb);

	//gluBeginSurface(theNurb);
	//gluNurbsSurface(theNurb, 6, knot, 4, knot1, 2 * 4, 4, &ctlarray3[0][0][0], 3, 2, GL_MAP2_VERTEX_4);
	//gluEndSurface(theNurb);
